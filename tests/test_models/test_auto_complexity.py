...
class TestEstimateRuntime:
    ...
    def test_more_qubits_slower(self) -> None:
        est_4 = estimate_runtime("kernel_svm", "tabular_quantum", 50, 4, {"epochs": 10, "batch_size": 16})
        est_8 = estimate_runtime("kernel_svm", "tabular_quantum", 50, 8, {"epochs": 10, "batch_size": 16})
        assert est_8 > est_4
...
class TestAutoSelectPresets:
    ...
    def test_tight_budget_selects_lower(self) -> None:
        config = ClaryonConfig(
            models=[ModelEntry(name="kernel_svm", type="tabular_quantum")],
            experiment={"name": "test", "complexity": "auto", "max_runtime_minutes": 1},
        )
        ...
        assert selected["kernel_svm"] in ("quick", "small", "medium")
    ...
    def test_generous_budget_selects_higher(self) -> None:
        ...
        assert selected["kernel_svm"] in ("large", "exhaustive")
