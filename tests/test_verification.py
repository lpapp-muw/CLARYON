...
class TestEndToEnd:
    """Verify the full pipeline runs on synthetic data."""

    def test_tabular_pipeline_produces_predictions(self) -> None:
        """Load tabular → split → train XGBoost → write predictions."""
        sys.path.insert(0, str(PROJECT_ROOT))
        train_path = FIXTURES_DIR / "tabular_binary" / "train.csv"
        if not train_path.exists():
            pytest.skip("Tabular fixtures missing")
        ...
        with tempfile.TemporaryDirectory() as td:
            try:
                ...
                from claryon.registry import get as reg_get
                model_cls = reg_get("model", "xgboost")
                model = model_cls(seed=42)

                model.fit(X_tr, y_tr, TaskType.BINARY)
                probs = model.predict_proba(X_te)
                assert probs.shape[0] == len(X_te)

                # Write predictions
                try:
                    from claryon.io.predictions import write_predictions
                    out_path = Path(td) / "predictions.csv"
                    ids = [f"S{i:04d}" for i in range(len(X_te))]
                    write_predictions(out_path, ids=ids, y_true=y_te, y_prob=probs)
                    assert out_path.exists()
                    raw = out_path.read_text()
                    assert ";" in raw.splitlines()[0]
                except Exception as e:
                    pytest.skip(f"Predictions write failed: {e}")
...
class TestGitHistory:
    ...
    def test_linear_history(self) -> None:
        ...
        merges = result.stdout.strip()
        assert merges == "", f"Merge commits found (should be linear): {merges}"

    def test_all_tests_pass(self) -> None:
        """Run the full existing test suite."""
        result = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/", "-q", "--timeout=600"],
            cwd=str(PROJECT_ROOT),
            capture_output=True, text=True
        )
