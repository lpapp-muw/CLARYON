...
# All phases complete. Default to 12 if WORKLOG.md not present.
PHASE=$(grep -oP 'CURRENT_PHASE:\s*\K\d+' WORKLOG.md 2>/dev/null || echo "12")
...
if [ "$PHASE" -ge 3 ]; then
    ...
    echo ""
    echo "--- TIER 4b: Quantum model smoke tests (kernel_svm, quantum_gp) ---"
