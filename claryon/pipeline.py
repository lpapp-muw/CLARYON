        ...
        for seed, splits in state.splits.items():
            for split in splits:
                ...
                try:
                    ...
                    # Imaging → tabular_quantum (amplitude-encoded quantum) image normalization.
                    # The "imaging" branch above only runs for 5D-tensor CNN models.
                    # For amplitude-encoded quantum models on flattened NIfTI we apply fixed_divisor / cohort_global
...
def stage_evaluate(config: ClaryonConfig, state: PipelineState, **kwargs: Any) -> None:
    ...
    from .evaluation import metrics as _metrics_module  # noqa: F401 — registers metrics
