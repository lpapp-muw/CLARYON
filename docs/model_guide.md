...
Amplitude-encoded models that compute a kernel matrix (`kernel_svm`, `qdc_hadamard`, `quantum_gp`) must compare every pair of samples. Doubling your sample count quadruples the runtime. For datasets with more than 500 samples, prefer `angle_pqk_svm` (O(N) circuits).
