"""QCNN ALT variant — WORK IN PROGRESS, NOT REGISTERED.
...
QCNN ALT variant — conv+pool layers with StronglyEntanglingLayers final block.

Ported from [E] pl_qcnn_alt.py. Input X must be amplitude-encoded.
"""
...
_warnings.warn(
    "claryon.models.quantum.qcnn_alt is WORK IN PROGRESS in CLARYON v0.13.0 "
...
# @register("model", "qcnn_alt")  # WIP — see top-of-module banner. Decorator
...
# maintainer.
class QCNNAltModel(ModelBuilder):
    """QCNN ALT-style model with conv+pool and StronglyEntanglingLayers.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._dense_layers = int(dense_layers)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, task_type: TaskType, **kwargs: Any) -> None:
        ...
        self._w_dense = pnp.array(
            rng.normal(0, scale, (self._dense_layers, self._n_final, 3)),
            requires_grad=True,
        )
        ...
        for epoch in range(self._epochs):
            logger.debug("qcnn_alt epoch %d/%d", epoch + 1, self._epochs)
            ...
            for start in range(0, N, bs):
                ...
                def cost(wk: Any, wd: Any) -> Any:
                    losses = pnp.stack([
                        -(yb[i] * pnp.log(self._qnode(xb[i], wk, wd) + 1e-8) +
                          (1 - yb[i]) * pnp.log(1 - self._qnode(xb[i], wk, wd) + 1e-8))
                        for i in range(len(b))
                    ])
                    return pnp.mean(losses)

                self._w_kernel, self._w_dense = opt.step(cost, self._w_kernel, self._w_dense)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        p1 = np.array([float(self._qnode(X[i], self._w_kernel, self._w_dense)) for i in range(X.shape[0])])
    ...
    def save(self, model_dir: Path) -> None:
        ...
        np.savez(model_dir / "weights.npz",
                 ...
                 w_dense=np.array(self._w_dense, dtype=np.float64))

    def load(self, model_dir: Path) -> None:
        import pennylane.numpy as pnp
        w = np.load(model_dir / "weights.npz")
        self._w_kernel = pnp.array(w["w_kernel"], requires_grad=False)
        self._w_dense = pnp.array(w["w_dense"], requires_grad=False)
