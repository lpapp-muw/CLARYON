...
class PLQCNN_MUW:
    """QCNN (MUW-style) operating on amplitude-encoded vectors.

    - Input X must be amplitude-encoded vectors of length pad_len (=2**n_qubits).
    - The circuit applies repeated (conv + pooling) layers that reduce the number of active wires.
    - A final qml.ArbitraryUnitary is applied on the remaining active wires.
    - Output is interpreted as P(class=1) via a projector measurement on the first active wire.
    """
    ...
    def _init_pl(self) -> None:
        ...
        def qnode(x: Any, wk: Any, wl: Any) -> Any:
            ...
            if int(pnp.size(wl)) != int(expected):
                raise ValueError(f"w_last wrong size expected={expected} got={pnp.size(wl)}")
    ...
    def fit(
        ...
        last_len = (4 ** int(self._n_final)) - 1
        self.w_last = pnp.array(rng.normal(0, scale, size=(last_len,)), requires_grad=True)
        ...
        for epoch in range(int(epochs)):
            self._log(f"[pl_qcnn_muw] epoch {epoch+1}/{int(epochs)}")
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        p1 = np.array(
            [float(self._qnode(X[i], self.w_kernel, self.w_last)) for i in range(X.shape[0])],
            dtype=np.float64,
        )
    ...
    def save(self, model_dir: Path, metadata: Dict[str, Any]) -> None:
        ...
        np.savez(
            ...
            w_last=np.array(self.w_last, dtype=np.float64),
        )

    @staticmethod
    def load(model_dir: Path) -> "PLQCNN_MUW":
        model_dir = Path(model_dir)
